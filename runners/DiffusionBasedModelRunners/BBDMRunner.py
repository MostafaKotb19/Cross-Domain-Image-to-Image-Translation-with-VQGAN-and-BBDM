...
class BBDMRunner(DiffusionBaseRunner):
    ...
    @torch.no_grad()
    def test_image(self, net, image_path, output_path):
        transform = transforms.Compose([
            transforms.Resize((self.config.data.dataset_config.image_size, self.config.data.dataset_config.image_size)),
            transforms.ToTensor()
        ])
        ...
        for i, image_file in enumerate(image_files):
            ...
            image_tensor = transform(image).unsqueeze(0).to(self.config.training.device[0])
            ...
            with torch.no_grad():
                output = net.sample(image_tensor, clip_denoised=False).squeeze(0).to('cpu')
                save_single_image(output, output_path, f'output.jpg', to_normal=to_normal)
