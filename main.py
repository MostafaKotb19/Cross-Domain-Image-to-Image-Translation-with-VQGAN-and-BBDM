...
from MaskGAN_demo.demo import Ex
from MaskGAN_demo.options.test_options import TestOptions
import sys
...
def parse_args_and_config():
    ...
    parser.add_argument('-i', '--test_image_path', type=str, default='images', help="The directory of input images")
    parser.add_argument('-o', '--output_path', type=str, default='output', help="The directory of output images")
    parser.add_argument('-u', '--ui', action='store_true', default=False, help="Use UI")
...
def ui_launcher(config):
    set_random_seed(config.args.seed)
    runner = get_runner(config.runner, config)
    return runner
    

def ui_tester(config, runner, test_image_path, output_path):
    if config.args.ui:
        with torch.no_grad():
            runner.ui(test_image_path, output_path)
